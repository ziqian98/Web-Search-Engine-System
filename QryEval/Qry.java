...
public abstract class Qry {
  ...
  public void appendArg(Qry q) throws IllegalArgumentException {
    ...
    /*
    NOTE:
    "this" refer to an operator here

    Qry queryTree = createOperator (substrings[0].trim());
    queryTree.appendArg (qargs[i]);

    queryTree is an operator and "this" refers operators
     */
   ...
   *  @return True if the query matches, otherwise false.
   */
  protected boolean docIteratorHasMatchFirst (RetrievalModel r) {  //called by #score docIteratorHasMatch()

    Qry q_0 = this.args.get(0);   //"apple"

    if (q_0.docIteratorHasMatch (r)) {  //"apple"
  ...
  protected boolean docIteratorHasMatchMin (RetrievalModel r) {
    ...
    for (int i=0; i<this.args.size(); i++) {   //search for all query args like "apple" ,"pie" under #OR
      Qry q_i = this.args.get(i);  //NOTE "this" refers to #OR if #OR is the object to call this method

      if (q_i.docIteratorHasMatch (r)) {  //only need one match so here we use if
        int q_iDocid = q_i.docIteratorGetMatch ();

        if ((minDocid > q_iDocid) ||  //Actually compare the first docid between "apple" and "pie"
    ...
    if (minDocid != Qry.INVALID_DOCID) {
      docIteratorSetMatchCache (minDocid);  //if #OR finds a minDocid, it will cache.
      return true;                          //getting score of a doc is after finding this doc
    } else {                                //which means we can find docid in cache when getting its score
      return false;
    }
  }
