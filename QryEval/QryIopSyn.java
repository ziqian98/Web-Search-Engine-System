...
public class QryIopSyn extends QryIop {
  ...
  protected void evaluate () throws IOException {
    ...
    //NOTE: This is Document at a time
    ...
    while (true) {
      ...
      for (Qry q_i: this.args) {  //given the minDocid, combine all term locations into the positions list
        if (q_i.docIteratorHasMatch (null) &&
            (q_i.docIteratorGetMatch () == minDocid)) {
          Vector<Integer> locations_i =
            ((QryIop) q_i).docIteratorGetMatchPosting().positions;  //get current doc's posting info
	  positions.addAll (locations_i);  //add all term locations in this matched minDocid
